...
export default function GalleryTabContent({ galleryImages, galleryImagesWithMeta, onImageClick }) {
     return (
          <FadeIn delay={100}>
               <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="text-center">
                         ...
                         <p className="text-gray-600 mt-1 font-medium">Bao gồm ảnh khu sân và tất cả sân nhỏ</p>
                    </div>
                    ...
                    {galleryImages && galleryImages.length > 0 ? (
                         <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                              {galleryImages.map((img, i) => {
                                   ...
                                   const meta = galleryImagesWithMeta && galleryImagesWithMeta[i];
                                   const isComplex = meta?.type === 'complex';
                                   const label = meta?.label || (isComplex ? 'Khu sân' : 'Sân nhỏ');
                                   ...
                                   return (
                                        <div key={i} className="relative group cursor-pointer overflow-hidden rounded-xl border border-teal-200/50 shadow-md hover:shadow-2xl transition-all hover:scale-[1.02]" onClick={() => onImageClick(i)}>
                                             ...
                                             {isComplex && (
                                                  <div className="absolute top-2 left-2 bg-gradient-to-r from-teal-500 to-emerald-500 text-white px-2 py-0.5 rounded-full text-xs font-bold shadow-lg border border-teal-400/30">
                                                       Khu sân
                                                  </div>
                                             )}
                                             {!isComplex && (
                                                  <div className="absolute top-2 left-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-2 py-0.5 rounded-full text-xs font-semibold shadow-lg border border-blue-400/30">
                                                       {label}
                                                  </div>
                                             )}
                                        </div>
