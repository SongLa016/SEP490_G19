...
export function useBookingPackages(user, playerId) {
  ...
  const normalizePackageData = useCallback((pkg, fallbackIndex = 0) => {
    ...
    return {
      ...
      qrCodeUrl:
        pkg?.qrcode || pkg?.qrCode || pkg?.QRCode || pkg?.qrCodeUrl || null,
      qrExpiresAt:
        pkg?.qrexpiresAt || pkg?.qrExpiresAt || pkg?.QRExpiresAt || null,
  ...
  const normalizePackageSession = useCallback((session) => {
    ...
    return {
      id:
        ...
        session.id ||
        `${packageId || "pkg"}-${
          session.sessionDate || session.date || Math.random()
        }`,
    ...
    try {
      ...
      if (apiResult.success) {
        ...
        if (apiResult.success) {
          ...
          setPackageError(
            apiResult.error || "Không thể tải lịch sử gói đặt sân cố định."
          );
        }
      } else {
        setPackageError(
      ...
      const normalizedPackages = packageList.map((pkg, index) =>
        normalizePackageData(pkg, index)
      );
      ...
      if (normalizedPackages.length > 0) {
        try {
          ...
          if (sessionResp.success && sessionResp.data) {
            ...
            const sessionsArray = Array.isArray(sessionResp.data)
              ? sessionResp.data
              : [];
            ...
            const schedulePromises = normalizedSessions
              ...
              .map(async (session) => {
                try {
                  ...
                  console.error(
                    `Error fetching schedule ${session.scheduleId}:`,
                    error
                  );
                }
                return null;
      ...
      setPackageError(
        error.message || "Không thể tải lịch sử gói đặt sân cố định."
      );
      setBookingPackages([]);
      setPackageSessionsMap({});
      setSessionScheduleDataMap({});
