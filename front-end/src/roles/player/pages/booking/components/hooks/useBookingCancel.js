    ...
    async (reason) => {
      ...
      try {
        ...
        if (isPending) {
          ...
          if (!result.success) {
            result = await cancelBookingAPI(
              bookingId,
              reason || "Hủy booking chưa được xác nhận"
            );
          } else {
        ...
        if (result.success) {
          ...
          if (finalScheduleId && Number(finalScheduleId) > 0) {
            try {
              await updateFieldScheduleStatus(
                Number(finalScheduleId),
                "Available"
              );
            } catch (error) {
              console.error(
                `Error updating schedule ${finalScheduleId}:`,
                error
              );
            }
          }
          ...
          const cleanReason = stripRefundQrInfo(
            refundInfo.cancelReason || result.data?.cancelReason || ""
          );
          ...
          let successHtml = `<p class="mb-3">${
            refundInfo.message || "Đã hủy booking thành công!"
          }</p>`;
          if (cleanReason) {
