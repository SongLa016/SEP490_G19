...
import { listBookingsByUser, fetchBookingsByPlayer } from "../../../../../../shared/index";
                    ...
                    .map(async (booking) => {
                         try {
                              ...
                              if (scheduleResult.success && scheduleResult.data) {
                                   return { scheduleId: booking.scheduleId, data: scheduleResult.data };
                              }
                         } catch (error) {
          ...
          const checkExpiredBookings = () => {
               ...
               setBookings(prevBookings => {
                    ...
                    prevBookings.forEach(booking => {
                         ...
                         const hasActiveQR = booking.qrExpiresAt && new Date(booking.qrExpiresAt).getTime() > currentTime;

                         if (!isPaid && (hasActiveQR || (isPendingOrConfirmed && isUnpaid && booking.createdAt))) {
                              let remaining = 0;
                              if (hasActiveQR) {
                    ...
                    if (hasExpiredBookings && !isReloadingRef.current) {
                         ...
                         fetchBookingsByPlayer(playerId).then(apiResult => {
                              ...
                              setTimeout(() => { isReloadingRef.current = false; }, 3000);
                         });
                    }
                    return prevBookings;
               });
